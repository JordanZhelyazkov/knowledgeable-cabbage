...
function getDefaultProjectFeatureFlags() {
    return {
        flatTree: true,
...
function getDefaultProjectWizard() {
    return {
        ssg: {
            id: 'nextjs',
            title: 'Next.js',
...
module.exports = {
    ...
    /*
     * TODO
     * - unset the ownerId if the project belongs to an organization
     * - give the creator permissions on the project they just created, otherwise they might be locked out of it immediately after creating it.
     */
    create: async (req, res) => {
        const user = req.user;
        const initialProject = { name: req.body.projectName };
        const token = req.body.token;
        const organizationId = req.body.organizationId;
        const themeId = req.body.themeId;
        if (organizationId && themeId) {
            const organization = await Organization.getOrganization(organizationId);
            const foundTheme = organization.registeredThemes.find((theme) => theme._id.toString() === themeId);
            if (foundTheme) {
                initialProject.wizard = {
                    theme: {
                        id: 'custom',
                        title: foundTheme.name,
                        settings: {
                            source: foundTheme.repoUrl,
                            image: foundTheme?.thumbnailUrl
                        }
                    }
                };
            } else {
        ...
        try {
            ...
            initialProject.widget = getDefaultProjectFeatureFlags();
            initialProject.wizard = { ...getDefaultProjectWizard(), ...initialProject.wizard };

            let project = await Project.createProject(initialProject, user, token);

            if (organizationId) {
                await Project.setOrganizationIdForProject(project.id, organizationId);
            } else {
                if (!customerTierService.getTierAttributes(project.subscription.tierId)?.isOpenToNewProjects) {
                    throw new Error('Invalid tier ID received from application');
                }
                    ...
                    return Project.createProject(
                        {
                            ...
                            settings: {
                                ...
                                // When projects are imported, since we don't support Netlify's DNS API (used when Netlify is also the DNS provider),
                                // assume that custom domains cannot safely be updated.
                                customDomainUpdatesEnabled: false
                            },
                            buildStatus: 'live',
                            status: 'live'
                        },
    ...
    updateProject: async (req, res) => {
        ...
        try {
            let project = await Project.updateProject(id, sanitizedProject, user.id);
            if (!project) {
                throw ResponseErrors.NotFound;
            }

            project = await deployments.callPureDeploymentMethodForProject('handleProjectUpdated', project, sanitizedProject, user);
            project = await Project.projectObjectForResponse(project, user);
            res.json(project);
        } catch (err) {
            res.status(err.status || 500).json({ message: err.data?.message || err.message });
        }
    },
            ...
            project = await Project.createProject(
                {
                    ...
                    buildStatus: 'draft',
                    widget: getDefaultProjectFeatureFlags(),
                    'wizard.theme': {
                        id: 'custom',
                        title: null,
                        settings: {
    ...
    makeAction: (req, res) => {
        ...
        return Project.findProjectByIdAndUser(projectId, req.user, permission)
            ...
            .catch((err) => {
                ...
                const error = isDescribedError
                    ? err.data
                    : ResponseErrors.ActionError({ message: err.message || _.startCase(`${action} failed`) });
    ...
    getStagedChanges: (req, res) => {
        ...
        return Project.findProjectByIdAndUser(projectId, user, CollaboratorRole.Permission.BASIC_ACCESS).then((project) => {
            return cmsTypes
                .baseInvokeContentSourcesWithProject('getStagedChanges', project, user, environmentName)
                .then((response) => {
                    return res.status(200).json(response);
                })
                .catch((err) => {
                    logger.error('getStagedChanges error', { error: err });
                    return res.status(err.status || 500).json({ message: 'Failed to get staged changes' });
                });
        });
    },
    ...
    // When a user adds a custom domain, they need setup that domain outside of Stackbit with whoever their DNS registrar.
    // After they've completed that process, the deployment implementation may need to close the circle.
    verifyCustomDomain: async (req, res) => {
        try {
            const user = req.user;
            const { id: projectId } = req.params;
            const project = await Project.findProjectByIdAndUser(projectId, user, CollaboratorRole.Permission.EDIT_ACCESS);
            ...
            await deployments.callPureDeploymentMethodForProject('verifyCustomDomain', project, user);
        } catch (error) {
